...
% ------------- BEGIN CODE --------------
    ...
    if size(vertexcolors, 2) == 1
        xPatch.type.VertexColorMesh = []; % this is a bit of a silly workaround. The Accord reader I am using in unity does not allow me to have data fields of string type, but reads out fieldnames as strings. So I am using the fieldname of a struct with no data to pass in the string "type". seems silly but it works
    else
